<table border="0" cellpadding="0" cellspacing="0" width="450px" class="table-form">
  	...
  	<tr>
  		...
  		<td width="300px">
  			...
  			maxlength="50" title="不超过100个字符" value="${task.name}"/>
  		</td>
  	</tr>
  	<tr>
  		...
  		<td>
  			...
  			title="<li>在进入任务时可以配置自定义的事件监听器来与流程引擎交互。</li><li>需要在spring配置文件中设置，编写请参考手册。</li>">
  				...
  				<#if (k == task.taskInListenerName)>
  				<option value="${k}" selected="selected">${taskInListener[k]}</option>
  				<#else>
  	...
  	<tr>
  		...
  		<td>
  			...
  			title="<li>在离开任务时可以配置自定义的事件监听器来与流程引擎交互。</li><li>需要在spring配置文件中设置，编写请参考手册。</li>">
  				...
  				<#if (k == task.taskOutListenerName)>
  				<option value="${k}" selected="selected">${taskOutListener[k]}</option>
  				<#else>
  	...
  	<tr>
  		...
  		<td>
  			<select id="actorType" name="actorType" style="width:200px;"
  				title="参与者类型需要用户自定义编写，配置到环境中可以选择。">
	 				<#list actorType?keys as k>
	 				<#if (k == task.actorType)>
	 				<option value="${k}" selected="selected">${actorType[k]}</option>
	 				<#else>
	 				</#if>
	 				</#list>
	 			</select>
  			...
  			<select id="multiUser" name="multiUser" style="width:200px;" onchange="changeMutiUser();"
  				title="对于有些如:会签，可以同时多个人参与，<br>系统会创建多个任务。">
	 				...
	 				<option value="0">否</option>
	 				  <#if (task.nodeTypeIndex == "singleTask")>
  	...
  	<tr>
  		...
  		<td>
  			<select id="actorExec" name="actorExec" style="width:200px;" title="任意一人完成表示只要一个人完成任务就向下继续流转">
	 				<#if (task.actorExec == 1)>
	 				<option value="0">任意一个人执行完</option>
	 				<option value="1" selected="selected">所有人执行完</option>
	 				<#else>
