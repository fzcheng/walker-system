<table border="0" cellpadding="0" cellspacing="0" width="450px" class="table-form">
 	 ...
 	 <tr>
  		...
  		<td width="300px">
  			<input type="text" id="pageUrl" name="pageUrl" class="text" value="${task.pageUrl}"
   ...
   <tr>
  		...
  		<td width="300px">
  			<input type="text" id="nextName" name="nextName" class="text" value="${task.nextName}" 
  			maxlength="50" title="可以在每个任务中自定义'下一步'按钮名称，如：发送。不超过50个字符"/>
  		</td>
  </tr>
  <#if (task.nodeTypeIndex != 'subProcessTask' && task.nodeTypeIndex != 'edgeTask')>
  <tr>
  		<td class="showTitle" width="150px">退回按钮名称：</td>
  		<td width="300px">
  			<input type="text" id="previousName" name="previousName" class="text"  value="${task.previousName}"
