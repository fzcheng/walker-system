...
this.xmlDoc=parser.parseFromString(xmlString,"text/xml")}catch(e){
this.xmlDoc=new ActiveXObject("Microsoft.XMLDOM");
...
this.xmlDoc.setRequestHeader("Content-type","text/xml")}else if(postMode)
...
dragger.dragNode=div.childNodes[0]}else
...
this.lastLanding_scr=(_isIE?e.srcElement:e.target)}else{
...
_isOpera=true;
_OperaRv=parseFloat(navigator.userAgent.substr(navigator.userAgent.indexOf('Opera')+6,3))}else if(navigator.appName.indexOf("Microsoft")!=-1){
_isIE=true;
...
return docObj.selectSingleNode(xpathExp)}else{
return docObj.selectNodes(xpathExp)||new Array(0)}}else{
var nodeObj=docObj;
if(!docObj){
if(!this.xmlDoc.nodeName){
...
c[c.length]=a[i].childNodes[j]}else if(a[i].childNodes[j].tagName==b)
...
var result=this.XSLProcessor.transformToDocument(xmlDoc)}else{
var result=new ActiveXObject("Msxml2.DOMDocument.3.0");
try{
xmlDoc.transformNodeToObject(xslDoc,result)}catch(e){
...
return xmlSerializer.serializeToString(xmlDoc)}else
return xmlDoc.xml};
