...
//		System.out.println(name + ":newpass:" + newpass);
	 ...
	 * @return
	 */
	public static int getDifferTimes(String begin) {
		if (begin == null) {
			return 0;
		}

		Date begin_time = formatDate(strToDate(begin), "yyyy-MM-dd HH:mm:ss");
		Date nowTime = new Date();
		nowTime = formatDate(nowTime, "yyyy-MM-dd HH:mm:ss");

		int days = 0;
		days = (int) ((nowTime.getTime() - begin_time.getTime()) / (MINUTE * 1000));
		return days;
	}
	...
	public static int getCurrentWeekDay_int(){
		Calendar c = Calendar.getInstance();
		c.setTime(new Date());
