...
this._pgn_templateA=this._pgn_template_compile("[prevpages:&lt:&nbsp] [currentpages:&nbsp:&nbsp] [nextpages:&gt:&nbsp]");
this._pgn_templateB=this._pgn_template_compile("当前显示:<b>[from]-[to]</b> &nbsp;总数据量:<b>[total]</b>")}};
...
return ds;
return '<a onclick=\'this.grid.changePageRelative(' + (mode == "next" ? '1' : (this.currentPage%this.pagesInGroup == 0 ? '1' : 'this.grid.pagesInGroup-this.grid.currentPage%this.grid.pagesInGroup+1')) + ');return false;\'>'+ac+'</a>'};
if(mode=="current"){
...
pageBox.className="dhx_page_active"+sfx}else{
var self=this;
