...
<body>
 	...
 	<form action="${ctx}/admin/user/edit.do" method="post" id="form" target="_self">
		<input type="hidden" id="parentId" name="parentId" value="${parentId}"/>
		<input type="hidden" id="roleIds" name="roleIds"/>
		<input type="hidden" id="id" name="id" value="${user.id}"/>
 	
 	<table border="0" cellpadding="0" cellspacing="0" width="600px" class="formtable">
   	<!-- 
   	<tr>
    ...
    -->
   	<tr>
   		<td class="title" width="150px">(<span class="required">*</span>)登录ID：</td>
   		<td width="450px">
   			<input type="text" id="loginId" name="loginId" class="textbox_req" value="${user.loginId}"
   			maxlength="30" title="只能输入英文、数字、下划线或者其组合，不超过30个字符"/>
   		</td>
   	</tr>
   	<tr>
   		<td class="title">(<span class="required">*</span>)用户名称：</td>
   		<td >
   			<input type="text" id="name" name="name" class="textbox_req" value="${user.name}"
   			maxlength="50"	title="用户姓名或者中文名称"/>
   		</td>
   	</tr>
   	<tr>
   		<td class="title">(<span class="required">*</span>)用户类型：</td>
   		<td width="260">
   			<select id="type" name="type"
   			title="<li>定义为单位管理员，那么该用户具有管理本单位机构和用户权限。</li><li>其可以看到本单位的部门和用户信息，这还要依赖权限是否给定。</li>">
   				<#if (user.type == 1)>
   					<option value="1" selected="selected">单位管理员</option>
   	...
   	<tr>
   		<td class="title">(<span class="required">*</span>)启用状态：</td>
   		<td width="260">
   	...
   	<tr>
   		...
   		<td >
   			<input type="text" id="summary" name="summary" class="textbox_req" maxlength="120"
   			 value="${user.summary}"	title="用户描述信息，不超过100字，可不填"/>
   		</td>
   	</tr>
   	<tr>
   		...
   		<div id="roleIdsDiv">
   			<#list roleList as role>
   				<#if (role.selected)>
	   				<input type="checkbox" name="checkId" value="${role.id}" checked="checked"/>
   				<#else>
